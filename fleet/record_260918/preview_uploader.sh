...
while true; do
  for d in "$P"/episodes/*coverage_walk*lv_*__v1r2s00*/; do
    ...
    SRC=""; for c in rgb.mp4 rgb_proxy.mp4; do [ -f "$d/$c" ] && { SRC="$d/$c"; break; }; done; [ -n "$SRC" ] || continue   # long episodes (>120k frames) only get the subsampled proxy
    if [ ! -f "$d/preview5.mp4" ]; then FF=$(ls /home/ubuntu/.local/lib/python3.10/site-packages/imageio_ffmpeg/binaries/ffmpeg-linux-x86_64-* 2>/dev/null | head -1); [ -n "$FF" ] && "$FF" -y -loglevel error -i "$SRC" -t 300 -c copy -movflags +faststart "$d/preview5.mp4" || { echo "$(date -u +%H:%M) ffmpeg failed $ep" >> $P/logs/preview_uploader.log; continue; }; fi
