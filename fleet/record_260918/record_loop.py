...
W = sys.argv[1]; B = 'pan-simworld'; PRE = 'ue-record/260918/'
F = Path('/home/ubuntu/ue_record_fleet_20260918'); P = Path('/home/ubuntu/WM-Unreal-data-collection/local_run/pipeline'); L = P / 'longvideo'
DST = 's3://pan-simworld/long-video-data-260918'
...
while True:
    ...
    try:
        ...
        rl = P / 'logs' / f'lv_{sid}_runner.log'
        if result['state'] not in ('accepted', 'rejected') and rl.exists() and 'apply_fill' in rl.read_text()[-20000:] and claimed.get('lighting') != 'level':
            # The fill sky-light capture hung the editor (17 Sep: indoor maps). Re-run once with the
            # author's lighting: a follow-up shard is appended to the queue with the same id + '_lvl'.
            fresh = get('manifest.json') or manifest
            if not any(t['shard_id'] == sid + '_lvl' for t in fresh):
                fu = dict(claimed, shard_id=sid + '_lvl', lighting='level', seed=int(claimed['seed']) + 100, fallback_of=sid)
                fresh.append(fu); put('manifest.json', fresh)
                tbl = json.loads((P / 'longvideo_shards.json').read_text()); tbl.append({k: v for k, v in fu.items() if k not in ('packs_to_sync', 'project_dir', 'est_s')}); (P / 'longvideo_shards.json').write_text(json.dumps(tbl, indent=1))
                log(f'{sid}: fill hung the editor; queued {sid}_lvl with lighting=level')
