...
try:
    ...
    # A task may raise the settle cap (quiet_max_s): heavy levels (CityPark 465 MB, Stadium) compile
    # shaders for 10+ min after PIE, and a request sent into that window is swallowed for good.
    QUIET_S = int(os.environ.get('QUIET_S', 45)); QUIET_MAX_S = int(task.get('quiet_max_s') or os.environ.get('QUIET_MAX_S', 900))
