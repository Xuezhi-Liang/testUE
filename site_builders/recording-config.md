...
- 路线每 45 m 一段，风格在 stroll / survey / inspect 之间轮换，所以一次覆盖沿途被用几种走法走过。第一遍按预算的 97% 配速，落后时暂时不做可选动作（看、扫、退）保证覆盖完成；后面几遍用 study / stroll 填满剩余预算。每遍从上一遍结束处接着走，不跳。
- 每集两遍覆盖，不封顶（09-17 改，原为 8 遍或 20 小时封顶）。Tokyo 实测相邻两遍耗时交替为巡游的 ~4× 与 ~8×，两遍合计可到一遍估计的 3×。
