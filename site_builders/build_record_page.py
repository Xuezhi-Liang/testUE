...
for c in sorted(cards, key=lambda c: (c['accepted'] is not True, c['slug'])):
    ...
    verdict = '<span class=ok>验收通过</span>' if c['accepted'] else (f"<span class=bad>验收未过：{', '.join(c['failed']) or '见 acceptance.json'}</span>" if c['accepted'] is False else '<span class=mut>未判</span>')
