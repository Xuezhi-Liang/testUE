...
h.append("</table>")
h.append(f"<p><b>A 类 {len(A)} 个项目的正式录制预算</b>：成片 {tot['ep']:.0f} h，{tot['sh']} 集，实时 2.5 倍计 {tot['mh']:.0f} 机时；10 台约 {tot['mh']/10/24:.1f} 天，20 台约 {tot['mh']/20/24:.1f} 天；存储约 {tot['gb']:,} GB。每集规则（09-17 改）：每张地图走两遍，一集，不封顶，不切分片。第一遍覆盖遍、第二遍 study 填充遍（约 2× 第一遍），成片按一遍估计的 3× 算。</p>")
