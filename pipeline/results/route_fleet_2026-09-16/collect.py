...
def fetch(obj):
 ...
 s3.download_file(B,key,str(tmp));tmp.replace(dest);seen[key]=signature
while True:
 try:
  objs=[]
  ...
  for i,x in enumerate(selected):
   ...
   for slug in allowed[w]:
    ...
    if r.get('state') in {'geometry_pass','coverage_review','route_failed','failed','blocked_asset','timeout'}:completed+=1
