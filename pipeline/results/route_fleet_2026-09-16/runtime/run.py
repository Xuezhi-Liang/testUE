...
STARTUP_S=1200;WORKER_MAX_S=7200;PROGRESS_IDLE_S=1800
  ...
  for startup_attempt in range(2):
   ...
   try:
    ...
    with elog.open('w') as ef:
     editor=subprocess.Popen(['enroot','exec',CTR_PID,'env','GAME_MAP='+mc['map_id'],'UNREALCV_PORT=9208','bash','/home/ubuntu/WM-Unreal-data-collection/local_run/launch_ue_fast.sh'],stdout=ef,stderr=subprocess.STDOUT,start_new_session=True)
    write(out/'process.json',dict(editor_pid=editor.pid,started_epoch=mt))
    native_log=Path('/proc')/CTR_PID/'root/home/ue4/simworld/Saved/Logs/gym_citynav.log'
    ...
    with wlog.open('w') as wf:
     worker=subprocess.Popen(['enroot','exec',CTR_PID,'env','PYTHONUNBUFFERED=1','OPENCV_IO_ENABLE_OPENEXR=1','python3',str(R/'worker.py'),slug],stdout=wf,stderr=subprocess.STDOUT,start_new_session=True)
