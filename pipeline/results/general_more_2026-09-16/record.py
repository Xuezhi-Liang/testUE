...
for tag,v in cfg['variants'].items():
 ...
 def progress():
  ...
  state={'map':name,'variant':tag,'frames':count,'total':n,'state':'录制中' if count else '照明校准 / 预热'}
 ...
 if tag=='baseline':
  summary=json.loads((ep/'capture_summary.json').read_text());cal=summary['lighting'].get('calibration') or {};factor=cal.get('chosen_factor')
  assert factor is not None,summary['lighting'];mc['fill']=float(factor);(out/'lighting_control.json').write_text(json.dumps({'factor':factor,'calibration':cal,'note':'Calibrated once using baseline; reuse this exact fill for the candidate.'},indent=2))
