...
opening=list(range(1,191,8));ids=sorted(set(opening+[i for v in groups.values() for i in v]));gx,gy=np.meshgrid(np.arange(1280),np.arange(720));full=(gx>20)&(gx<1260)&(gy>20)&(gy<700);flow={};mask={}
...
for tag in tags:
 ...
 building_static=[i for i in opening if base[i]['actual_location']==base[i+1]['actual_location']]
