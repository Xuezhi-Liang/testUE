...
SITE=Path('/home/ubuntu/WM-Unreal-data-collection/local_run/site/longvideo/new-map-validation')
...
def publish():
 ...
 atomic(SITE/'status.json',dict(updated_epoch=time.time(),queue=q,counts=counts,fleet=load(R/'fleet.json',{}),total=len(rows),completed=sum(counts.get(k,0) for k in TERMINAL),rows=rows))
