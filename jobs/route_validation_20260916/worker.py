...
try:
 ...
 spdir=out/'starts';spdir.mkdir(exist_ok=True);source=P.parent.parent/'batch_inference/start_positions'/f'{slug}.json';sp=json.loads(source.read_text());valid=[x for x in sp.get('positions',[]) if all(isinstance(x.get(k),(int,float)) for k in ['x','y','z'])]
 if mc['recover_start']:
...
""")
 ...
 for attempt in range(2):
  task=json.loads((R/'template.json').read_text());task.update(task_id=f'routecheck_20260916_{slug}_{attempt}',map_id=mc['map_id'],seed=mc['seed']+attempt*1000,target_passes=1,max_duration_s=72000,allow_short_episode=True,min_frames=240,route_region_policy=mc['policy']);task['body']['ground_clearance_cm']=mc['ground_clearance_cm'];task['camera']['pitch_limit_up_deg']=float(mc.get('pitch_limit_up_deg',task['camera']['pitch_limit_up_deg']))
